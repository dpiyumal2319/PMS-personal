...
const feeTypes: Record<ChargeType, {
    icon: LucideIcon,
    color: string,
    suffix?: string,
    description: string,
    type: 'compulsory' | 'selective'
}> = {
    FIXED: {
        icon: CreditCard,
        color: "blue",
        description: "Fixed fees will add to each patient's bill regardless of the total bill amount",
        suffix: 'LKR',
        type: 'compulsory'
    },
    PERCENTAGE: {
        ...
        color: "green",
        suffix: "%",
        description: "Percentage fees will be calculated based on the total bill amount on each patient's bill",
        type: 'compulsory'
    },
    DISCOUNT: {
        icon: Tag,
...
const FeesCard = ({feeValues, handleInputChange, type, handleDeleteFee}: {
    ...
    const {icon: Icon, color, suffix, description} = feeTypes[type];
...
// Export specific fee type components
