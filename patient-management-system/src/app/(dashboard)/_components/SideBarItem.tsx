    ...
    return (<div className="flex flex-col">
            ...
            >
                ...
                <span className="ml-auto">
                    <FaChevronDown
                        className={`text-white transform transition-all duration-300 ease-in-out origin-center
            ...
            <div
                className={`ml-6 flex flex-col transition-all duration-300 ease-in-out ${expanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}
            >
                {expanded && item.links.map((link) => (<SideBarItem key={link.name} item={link}/>))}
            </div>
        </div>);
}