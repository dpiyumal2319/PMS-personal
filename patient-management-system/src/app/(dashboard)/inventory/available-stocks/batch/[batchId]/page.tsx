// components/BatchDetailsPage.tsx
...
import BatchStatusChangeButtonBar from '../../_components/BatchStatusChangeButtonBar';
...
export default async function BatchDetailsPage({
    ...
    const batchData = await getBatchData(parseInt((resolvedParams).batchId));


    const handleStatusChange = (status: string) => {
        alert(`Batch status changed to ${status}`);
    };
    ...
    return (
        <div className="flex flex-col w-full mx-auto p-4">
            ...
            <div className="flex justify-end gap-2 mt-6">
                <BatchStatusChangeButtonBar batchId={resolvedParams.batchId} />
            </div>
        </div>
    );
};
